...
* [README_linux.md](https://github.com/GonzaBot/Instalar-IA-local-deepseek-Windows-linux/blob/main/README_linux.md) — instalación completa en Linux (probado en Linux Mint / Ubuntu).
* [README_windows.md](https://github.com/GonzaBot/Instalar-IA-local-deepseek-Windows-linux/blob/main/README_windows.md) — instalación completa en Windows 10/11.
* [README_windows_intel_igpu.md](https://github.com/GonzaBot/Instalar-IA-local-deepseek-Windows-linux/blob/main/README_windows_intel_igpu.md) — Windows con aceleración real por GPU integrada Intel Iris Xe (para cuando el setup estándar de Windows deja todo el peso en la CPU).

El comando para descargar y correr el modelo (`ollama pull deepseek-r1:7b` / `ollama run deepseek-r1:7b`) es idéntico en los tres casos. Lo que cambia entre guías es cómo se instala Ollama, cómo se gestiona el servicio en segundo plano, el setup de Docker para la interfaz web opcional, y — en el caso de Intel iGPU — el backend SYCL necesario para que la GPU integrada se use de verdad en vez de descartarse. Cada guía lo cubre de punta a punta para su caso.
